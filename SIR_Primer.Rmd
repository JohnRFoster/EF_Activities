...
* [EcoForecast Hands on Activites](https://github.com/EcoForecast/EF_Activities)

## Why Forecast?

![](images/ForecastUmbrella.jpg)

***

![](images/DecisionsAboutFuture.jpg)

***

![](images/ICC_Uncertainty.jpg)

***

![](images/ForecastUpdate.jpg)

***

![](images/ForecastCycleDietze2018.jpg)

***

![](images/ForecastAprioriOutOfSample.jpg)

***

![](images/RealTimeMonitoring.jpg)

***

![](images/NetworkScience.jpg)

***

![](images/HowDoWeMeasurePredictability.jpg)

***

![](images/UncertaintySourcesWithPics.jpg)

***

![](images/WhyForecastSummary.jpg)
...
$$Y_{t+1} = f(Y_t,X_t | \theta + \alpha) + \epsilon$$   
...
$$Var[Y_{t+1}] \approx \underbrace{\left (\frac{\partial f}{\partial Y}\right )^2 Var[Y_t]}_{Initial \; Conditions}
 ...
 + \underbrace{Var[\epsilon]}_{error}$$ 
...
lines(rseq, dlnorm(rseq, meanlog = log(r0), sdlog = exp(r0) * 0.1), col =
        "red")
```
...
forecastSIR <- function(IC, r, beta, NT) {
  ## if r and beta are not time varying, make them all the same
...
## dimensions = c(ensembles, time, states)
...
```{r}
## calculate CI
SIR_Eci = apply(SIR_E, 3, function(x) {apply(x, 2, quantile, c(0.025, 0.5, 0.975))})  
...
for (i in 1:Nmc) {
  ## note that IC is now indexed by run [i]
  SIR_EI[i, , ] = forecastSIR(IC[i, ], rbar, bbar, NT)  
}
```
...
```{r}
## calculate CI
SIR_EIci = apply(SIR_EI, 3, function(x) {apply(x, 2, quantile, c(0.025, 0.5, 0.975))})  
SIR_EIci[SIR_EIci == 0] <- 0.1 ## sub just for log-scale plotting

# plot colors
COLS = c('#d7191c', '#fdae61', '#abd9e9', '#2c7bb6')

plot(time, SIR_Eci[ind + 1, 2],
  type = 'l',
  log = 'y',
  ylim = c(1, sum(ICbar)),
  xlab = "days",
  ylab = "Population (log scale)",
  lwd = 3)
ciEnvelope(time, SIR_EIci[ind, 2], SIR_EIci[ind + 2, 2], col = COLS[2])
ciEnvelope(time, SIR_Eci[ind, 2], SIR_Eci[ind + 2, 2], col = COLS[1])
lines(time, SIR_Eci[ind + 1, 2], lwd = 3)
legend("topright",
  legend = c("Proc", "Proc+IC"),
  lwd = 3,
  col = COLS)
```
...
for (i in 1:Nmc) {
   ## note that r and beta are now indexed by run [i]
  SIR_EIP[i, , ] = forecastSIR(IC[i, ], r[i], beta[i], NT) 
}
```

```{r}
## calculate CI
SIR_EIPci = apply(SIR_EIP, 3, function(x) {apply(x, 2, quantile, c(0.025, 0.5, 0.975))})
...
plot(time, SIR_Eci[ind + 1, 2],
  ...
  ylab = "Population (log scale)",
  lwd = 3)
ciEnvelope(time, SIR_EIPci[ind, 2], SIR_EIPci[ind + 2, 2], col = COLS[3])
ciEnvelope(time, SIR_EIci[ind, 2], SIR_EIci[ind + 2, 2], col = COLS[2])
ciEnvelope(time, SIR_Eci[ind, 2], SIR_Eci[ind + 2, 2], col = COLS[1])
lines(time, SIR_Eci[ind + 1, 2], lwd = 3)
legend("topright",
  legend = c("Proc", "Proc+IC", "Proc+IC+Param"),
  lwd = 3,
  col = COLS)
```
...
for (i in 1:Nmc) {
  ## note the existing beta and r being mutated
  SIR_EIPA[i, , ] = forecastSIR(IC[i, ], r[i] * mutate_r[i, ], beta[i] * mutate_b[i, ], NT)  
}
```

```{r}
## calculate CI
SIR_EIPAci = apply(SIR_EIPA, 3, function(x) {apply(x, 2, quantile, c(0.025, 0.5, 0.975))})  
...
plot(time, SIR_Eci[ind + 1, 2],
  ...
  ylab = "Population (log scale)",
  lwd = 3)
ciEnvelope(time, SIR_EIPAci[ind, 2], SIR_EIPAci[ind + 2, 2], col = COLS[4])
ciEnvelope(time, SIR_EIPci[ind, 2], SIR_EIPci[ind + 2, 2], col = COLS[3])
ciEnvelope(time, SIR_EIci[ind, 2], SIR_EIci[ind + 2, 2], col = COLS[2])
ciEnvelope(time, SIR_Eci[ind, 2], SIR_Eci[ind + 2, 2], col = COLS[1])
lines(time, SIR_Eci[ind + 1, 2], lwd = 3)
legend("topright",
  legend = c("Proc", "Proc+IC", "Proc+IC+Param", "Full"),
  lwd = 3,
  col = COLS)
```
...
# relative variances (var sum to 1)
V.rel <- apply(varMat, 2, function(x) {x / max(x)})

plot(time, V.rel[1, ],
  ylim = c(0, 1),
  type = 'n',
  main = "Relative Variance",
...
legend("bottom",
  legend = c("Parm Var", "Parm Unc", "InitCond", "Process"),
  col = rev(COLS),
  lty = 1,
  lwd = 5)
```
